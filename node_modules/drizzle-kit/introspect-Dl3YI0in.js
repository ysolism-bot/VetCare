...
const fromDatabase = async (db, filter = () => true, progressCallback = () => {}, queryCallback = () => {}) => {
  ...
  `).then((rows) => {
	...
	for (const column of columnsList.filter((x) => x.kind === "r" || x.kind === "p")) {
		...
		const metadata = typeof column.metadata === "string" ? JSON.parse(column.metadata) : column.metadata;
	...
	for (const idx of idxs) {
		const metadata = typeof idx.metadata === "string" ? JSON.parse(idx.metadata) : idx.metadata;
	...
	return {
		...
		roles: resultRoles,
		privileges,
		policies: policies.filter((p) => resultTables.some((t) => t.schema === p.schema && t.name === p.table)),
		views: resultViews,
