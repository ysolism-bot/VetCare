...
const require_snapshot = require('./snapshot-CHMNRJxo.js');
const require_brace_expansion$1 = require('./brace-expansion-CQ04dhdS.js');
...
var require_fs_realpath = /* @__PURE__ */ require_chunk.__commonJSMin(((exports, module) => {
...
var require_common = /* @__PURE__ */ require_chunk.__commonJSMin(((exports) => {
	...
	function isIgnored(self, path$6) {
		if (!self.ignore.length) return false;
		return self.ignore.some(function(item) {
			return item.matcher.match(path$6) || !!(item.gmatcher && item.gmatcher.match(path$6));
		});
	}
	function childrenIgnored(self, path$7) {
		if (!self.ignore.length) return false;
		return self.ignore.some(function(item) {
			return !!(item.gmatcher && item.gmatcher.match(path$7));
		});
	}
}));
...
const Binary$1 = {
	...
	defaultFromIntrospect: (value) => {
		if (value.startsWith("0x")) {
			if (typeof Buffer !== "undefined") return `'${Buffer.from(value.slice(2), "hex").toString("utf-8")}'`;
			const matches = value.slice(2).match(/.{1,2}/g) ?? [];
...
const NVarchar = {
	...
	defaultFromDrizzle: (value) => {
		...
		else if (typeof value === "object" || Array.isArray(value)) result = require_grammar.stringify(value, (_, value) => {
