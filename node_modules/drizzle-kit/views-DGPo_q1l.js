...
const require_utils$1 = require('./utils-D9CsqMfV.js');
const require_grammar = require('./grammar-Dw-D_yEp.js');
const require_types = require('./types-B0Upgohz.js');
...
//#endregion
//#region ../node_modules/.pnpm/hanji@0.0.8/node_modules/hanji/readline.js
...
//#endregion
//#region ../node_modules/.pnpm/hanji@0.0.8/node_modules/hanji/utils.js
var require_utils = /* @__PURE__ */ require_chunk.__commonJSMin(((exports) => {
	...
	exports.strip = strip;
	const stripAnsi = (str) => {
		if (typeof Bun !== "undefined" && Bun.stripANSI) return Bun.stripANSI(str);
		return (0, exports.strip)(str);
	};
	exports.stripAnsi = stripAnsi;
	const fallbackStringWidth = (str) => {
		let len = 0;
	...
	const clear = function(prompt, perLine) {
		...
		for (let line of lines) rows += 1 + Math.floor(Math.max((0, exports.stringWidth)(line) - 1, 0) / perLine);
...
var require_lodash_throttle = /* @__PURE__ */ require_chunk.__commonJSMin(((exports, module) => {
	/**
...
//#endregion
//#region ../node_modules/.pnpm/hanji@0.0.8/node_modules/hanji/index.js
var require_hanji = /* @__PURE__ */ require_chunk.__commonJSMin(((exports) => {
	...
	const lodash_throttle_1 = __importDefault(require_lodash_throttle());
	var Prompt = class {
		constructor() {
	...
	function render(view) {
		...
		if (!process.stdin.isTTY || !process.stdout.isTTY) return Promise.reject(/* @__PURE__ */ new Error("Interactive prompts require a TTY terminal (process.stdin.isTTY or process.stdout.isTTY is false). This can happen when running in CI, piped input, or non-interactive shells."));
		const closable = (0, readline_1.createClosable)();
		const terminal = new Terminal(view, readline_1.stdin, readline_1.stdout, closable);
...
const entitiesParams = {
	...
	extensionsFilters: require_types.literalType("postgis").array().optional(),
	entities: require_types.objectType({ roles: require_types.booleanType().or(require_types.objectType({
		provider: require_types.stringType().optional(),
		include: require_types.stringType().array().optional(),
		exclude: require_types.stringType().array().optional()
...
const configPull = configCommonSchema.extend({
	casing,
	migrations: configMigrations,
	...entitiesParams
});
const configGenerate = configCommonSchema.extend({ schema: require_types.unionType([require_types.stringType(), require_types.stringType().array()]) });
const configPush = configCommonSchema.extend({
	schema: require_types.unionType([require_types.stringType(), require_types.stringType().array()]),
	explain: require_types.booleanType().optional(),
	migrations: configMigrations,
	...entitiesParams
});
const configExport = configCommonSchema.extend({ schema: require_types.unionType([require_types.stringType(), require_types.stringType().array()]) });
const studioCliParams = require_types.objectType({
	port: require_types.coerce.number().optional().default(4983),
	host: require_types.stringType().optional().default("127.0.0.1"),
	config: require_types.stringType().optional()
});
const configStudio = configCommonSchema.extend({ schema: require_types.unionType([require_types.stringType(), require_types.stringType().array()]).optional() });
const configMigrate = configCommonSchema.extend({ migrations: configMigrations });

//#endregion
//#region src/cli/validations/outputs.ts
const withStyle = {
	error: (str) => `${chalk.red(`${chalk.white.bgRed(" Invalid input ")} ${str}`)}`,
	warning: (str) => `${chalk.white.bgGray(" Warning ")} ${str}`,
	errorWarning: (str) => `${chalk.red(`${chalk.white.bgRed(" Warning ")} ${str}`)}`,
	fullWarning: (str) => `${chalk.black.bgYellow(" Warning ")} ${chalk.bold(str)}`,
	suggestion: (str) => `${chalk.white.bgGray(" Suggestion ")} ${str}`,
	info: (str) => `${chalk.grey(str)}`
};

//#endregion
//#region src/cli/views.ts
var views_exports = /* @__PURE__ */ require_chunk.__exportAll({
	ResolveSelect: () => ResolveSelect,
	err: () => err,
	isRenamePromptItem: () => isRenamePromptItem,
	postgresSchemaError: () => postgresSchemaError,
	postgresSchemaWarning: () => postgresSchemaWarning
});
var import_hanji = require_hanji();
const err = (msg) => {
	(0, import_hanji.render)(`${chalk.bold.red("Error")} ${msg}`);
};
...
const postgresSchemaError = (error) => {
	...
	if (error.type === "pgvector_index_noop") {
		...
		return withStyle.errorWarning(`You are specifying an index on the ${chalk.blueBright(column)} column inside the ${chalk.blueBright(table)} table with the ${chalk.blueBright("vector")} type without specifying an operator class. Vector extension doesn't have a default operator class, so you need to specify one of the available options. Here is a list of available op classes for the vector extension: [${require_grammar.vectorOps.map((it) => `${chalk.underline(`${it}`)}`).join(", ")}].\n\nYou can specify it using current syntax: ${chalk.underline(`index("${indexName}").using("${method}", table.${column}.op("${require_grammar.vectorOps[0]}"))`)}\n\nYou can check the "pg_vector" docs for more info: https://github.com/pgvector/pgvector?tab=readme-ov-file#indexing\n`);
	}
	if (error.type === "policy_duplicate") {
		const { schema, table, policy } = error;
		const tableName = chalk.underline.blue(`"${schema}"."${table}"`);
		return withStyle.errorWarning(`We've found duplicated policy name across ${tableName} table. Please rename one of the policies with ${chalk.underline.blue(policy)} name`);
	}
...
var ResolveSelect = class extends import_hanji.Prompt {
	...
	render(status) {
		...
		const selectedPrefix = isRenamePromptItem(this.state.items[this.state.selectedIdx]) ? chalk.yellow("❯ ") : chalk.green("❯ ");
		...
		this.state.items.forEach((it, idx) => {
			...
			const label = isRenamed ? `${chalk.yellow("~")} ${title} ${chalk.gray(`rename ${entityType}`)}` : `${chalk.green("+")} ${title} ${chalk.gray(`create ${entityType}`)}`;
...
Object.defineProperty(exports, 'views_exports', {
  enumerable: true,
  get: function () {
    return views_exports;
  }
});
