...
function lineAndColumnToMessage(...ranges) {
	...
	const { repeatStr } = common_exports;
...
var Grammar = class Grammar {
	...
	_checkTopDownActionDict(what, name, actionDict) {
		...
		if (problems.length > 0) {
			...
			const error = new Error([`Found errors in the action dictionary of the '${name}' ${what}:`, ...prettyProblems].join("\n"));
...
var GrammarDecl = class {
	...
	installOverriddenOrExtendedRule(name, formals, body, source) {
		const duplicateParameterNames$2 = getDuplicates(formals);
		if (duplicateParameterNames$2.length > 0) throw duplicateParameterNames(name, duplicateParameterNames$2, source);
		const ruleInfo = this.ensureSuperGrammar().rules[name];
...
var Builder = class {
	...
	grammar(metaInfo, name, superGrammar, defaultStartRule, rules) {
		...
		Object.keys(rules).forEach((ruleName) => {
			...
			let source;
			if (gDecl.source && metaInfo && metaInfo.sourceInterval) source = gDecl.source.subInterval(metaInfo.sourceInterval[0], metaInfo.sourceInterval[1] - metaInfo.sourceInterval[0]);
	...
	fromRecipe(recipe) {
		...
		if (metaInfo) {
			if (metaInfo.sourceInterval && this.currentDecl) result.withSource(this.currentDecl.sourceInterval(...metaInfo.sourceInterval));
		}
		return result;
	}
};
...
const literalArraySemantics = result.PGArrayLiteral.createSemantics();
...
const expressionArraySemantics = result.PGArrayExpression.createSemantics();
...
function parseArray(array) {
	const match = result.PGArrayLiteral.match(array, "Array");
...
function parseExpressionArray(array) {
	const match = result.PGArrayExpression.match(array, "Array");
	if (match.failed()) throw new Error(`Failed to parse array: '${array}'`);
...
const newParse = (p_user_options) => {
	...
	const pObject = (schema) => {
		const result = p_options.protoAction === preserve ? Object.create(null) : {};
		if (p_current_char === `{`) {
			pNext();
			pSkipWhite();
			if (p_current_char === `}`) {
			...
			while (p_current_char) {
				...
				pCurrentCharIs(`:`);
				pNext();
				if (p_options.errorOnDuplicatedKeys === true && Object.hasOwnProperty.call(result, key)) pError(`Duplicate key "${key}"`);
				...
				else result[key] = pJsonValue(sub_schema);
	...
	const pArray = (schema) => {
		...
		if (p_current_char === `[`) {
			...
			while (p_current_char) {
				result.push(pJsonValue(is_tuple_like ? schema[result.length] : is_array ? schema[0] : void 0));
				pSkipWhite();
				if (p_current_char === `]`) {
					pNext();
					return result;
	...
	const pString = () => {
		...
		if (p_current_char === `"`) {
			...
			while (pNext()) {
				if (p_current_char === `"`) {
					if (p_current_char_index > start_at) result += p_text.substring(start_at, p_current_char_index);
					pNext();
					return result;
				}
				if (p_current_char === `\\`) {
					...
					} else if (typeof ESCAPEE[p_current_char] === `string`) result += ESCAPEE[p_current_char];
					else break;
	...
	const pNumber = (() => {
		...
		return (schema) => {
			...
			const result = cache.get(result_string).get(raw_schema);
		...
		switch (typeof value) {
			...
			case `object`: {
				...
				if (Array.isArray(value)) {
					const partial = value.map((_v_, i) => sStringify(value, i, delim, n) || `null`);
...
const filterMigrationsSchema = (interim, migrations) => {
	...
	if (interim.defaults) interim.defaults = interim.defaults.filter((def) => !(def.table === migrationsTable && (def.schema ? def.schema === migrationsSchema : true)));
	if (interim.schemas) {
...
const Char = {
	...
	defaultArrayFromDrizzle: (value) => {
		return `'${require_utils.stringifyArray(value, "sql", (v) => {
			...
			if (v.includes("\\") || v.includes("\"") || v.includes(",") || v.length === 0) return `"${escaped}"`;
			return escaped;
		})}'`;
	},
	defaultFromIntrospect: (value) => {
...
const Json = {
	...
	drizzleImport: () => "json",
	defaultFromDrizzle: (value) => {
		return `'${stringify(value, (_, value) => {
	...
	toTs: (_, value) => {
		...
		try {
			return { default: stringify(parse(trimmed), (_, value) => {
				if (typeof value !== "string") return value;
				return value.replaceAll("''", "'");
			}, void 0, true) };
		} catch {}
...
const isSerialExpression = (expr, schema) => {
	...
	return (expr.startsWith(`nextval('${schemaPrefix}`) || expr.startsWith(`nextval('"${schemaPrefix}"${schemaPrefix ? "." : ""}`)) && (expr.endsWith(`_seq'::regclass)`) || expr.endsWith(`_seq"'::regclass)`));
};
