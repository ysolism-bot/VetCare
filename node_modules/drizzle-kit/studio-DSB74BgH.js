...
const zValidator = (target, schema, hook) => validator(target, async (value, c) => {
...
const getCustomDefaults = (schema) => {
...
const drizzleForPostgres = async (credentials, pgSchema, relations, schemaFiles) => {
	const { preparePostgresDB } = await Promise.resolve().then(() => require("./connections-CZrg3hIX.js"));
...
const drizzleForMySQL = async (credentials, mysqlSchema, relations, schemaFiles) => {
	const { connectToMySQL } = await Promise.resolve().then(() => require("./connections-CZrg3hIX.js"));
...
const drizzleForSQLite = async (credentials, sqliteSchema, relations, schemaFiles) => {
	...
	if ("driver" in credentials && credentials.driver === "d1") {
		const { connectToD1 } = await Promise.resolve().then(() => require("./connections-CZrg3hIX.js"));
	...
	const { connectToSQLite } = await Promise.resolve().then(() => require("./connections-CZrg3hIX.js"));
...
const extractRelations = (tablesConfig) => {
...
const prepareServer = async ({ dialect, driver, packageName, databaseName, proxy, transactionProxy, benchmarkProxy, customDefaults, schema: drizzleSchema, relations, dbHash, schemaFiles }, app) => {
	...
	const relationsConfig = (0, drizzle_orm__relations.extractTablesRelationalConfig)({
		...Object.fromEntries(Object.entries(drizzleSchema).map(([schemaName, schema]) => {
