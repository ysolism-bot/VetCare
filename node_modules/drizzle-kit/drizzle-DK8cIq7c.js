...
var drizzle_exports = /* @__PURE__ */ require_chunk.__exportAll({
	defaultFromColumn: () => defaultFromColumn,
	fromDrizzleSchema: () => fromDrizzleSchema,
...
const fromDrizzleSchema = (schema, filter) => {
	const dialect = new drizzle_orm_pg_core.PgDialect();
	const errors = [];
	const warnings = [];
	const res = {
		indexes: [],
	...
	res.tables = tableConfigPairs.map((it) => {
		...
		const isRlsEnabled = config.enableRLS || config.policies.length > 0 || res.policies.some((x) => x.schema === schema && x.table === config.name);
		return {
			entityType: "tables",
			schema,
			name: config.name,
	...
	const combinedViews = [...schema.views, ...schema.matViews].sort((a, b) => {
		const aConfig = (0, drizzle_orm.is)(a, drizzle_orm_pg_core.PgView) ? (0, drizzle_orm_pg_core.getViewConfig)(a) : (0, drizzle_orm_pg_core.getMaterializedViewConfig)(a);
		const bConfig = (0, drizzle_orm.is)(b, drizzle_orm_pg_core.PgView) ? (0, drizzle_orm_pg_core.getViewConfig)(b) : (0, drizzle_orm_pg_core.getMaterializedViewConfig)(b);
	...
	for (const view of combinedViews) {
		...
		const { name: viewName, schema, query, tablespace, using, withNoData, materialized } = view;
...
const fromExports = (exports) => {
	...
	const matViews = [];
	const relations = [];
	Object.values(exports).forEach((t) => {
