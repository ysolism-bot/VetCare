...
const convertor = (type, convertor) => {
	return {
		type,
		can: (st) => {
			return st.type === type;
...
const alterViewConvertor = convertor("alter_view", (st) => {
	...
	const setOptions = Object.entries(withTo).filter(([key, val]) => {
		const from = key in withFrom ? withFrom[key] : null;
	...
	if (diff.tablespace) {
		const to = diff.tablespace.to || require_grammar.defaults.tablespace;
		statements.push(`ALTER ${viewClause} SET TABLESPACE "${to}";`);
	}
	if (diff.using) {
		const toUsing = diff.using.to ?? require_grammar.defaults.accessMethod;
...
const alterColumnConvertor = convertor("alter_column", (st) => {
	...
	if (diff.type) {
		...
		const type = diff.typeSchema?.to && diff.typeSchema.to !== "public" ? `"${diff.typeSchema.to}"."${diff.type.to}"` : isEnum ? `"${diff.type.to}"` : toSerial ? require_grammar.mapSerialToInt(diff.type.to) : diff.type.to;
		if (wasSerial) {
	...
	if (diff.default && !recreateDefault) if (diff.default.to) statements.push(`ALTER TABLE ${key} ALTER COLUMN "${column.name}" SET DEFAULT ${require_grammar.defaultToSQL(diff.$right)};`);
	else statements.push(`ALTER TABLE ${key} ALTER COLUMN "${column.name}" DROP DEFAULT;`);
	if (diff.generated && diff.generated.to === null) statements.push(`ALTER TABLE ${key} ALTER COLUMN "${column.name}" DROP EXPRESSION;`);
	if (diff.identity) if (diff.identity.from === null) {
	...
	} else if (diff.identity.to === null) statements.push(`ALTER TABLE ${key} ALTER COLUMN "${column.name}" DROP IDENTITY;`);
	else {
...
const dropIndexConvertor = convertor("drop_index", (st) => {
	...
	return `DROP INDEX ${schema !== "public" ? `"${schema}".` : ""}"${st.index.name}";`;
});
...
const alterEnumConvertor = convertor("alter_enum", (st) => {
	...
	const statements = [];
	for (const d of diff.filter((it) => it.type === "added")) if (d.beforeValue) statements.push(`ALTER TYPE ${key} ADD VALUE '${d.value}' BEFORE '${d.beforeValue}';`);
...
const alterRoleConvertor = convertor("alter_role", ({ diff, role }) => {
	const { name } = role;
	return `ALTER ROLE "${name}" WITH${diff.superuser ? diff.superuser.to ? " SUPERUSER" : " NOSUPERUSER" : ""}${diff.createDb ? diff.createDb.to ? " CREATEDB" : " NOCREATEDB" : ""}${diff.createRole ? diff.createRole.to ? " CREATEROLE" : " NOCREATEROLE" : ""}${diff.inherit ? diff.inherit.to ? " INHERIT" : " NOINHERIT" : ""}${diff.canLogin ? diff.canLogin.to ? " LOGIN" : " NOLOGIN" : ""}${diff.replication ? diff.replication.to ? " REPLICATION" : " NOREPLICATION" : ""}${diff.bypassRls ? diff.bypassRls.to ? " BYPASSRLS" : " NOBYPASSRLS" : ""}${diff.connLimit ? typeof diff.connLimit.to === "number" ? ` CONNECTION LIMIT ${diff.connLimit.to}` : " CONNECTION LIMIT -1" : ""}${diff.password ? diff.password.to ? ` PASSWORD '${require_utils.escapeSingleQuotes(diff.password.to)}'` : " PASSWORD NULL" : ""}${diff.validUntil ? diff.validUntil.to ? ` VALID UNTIL '${diff.validUntil.to}'` : ` VALID UNTIL 'infinity'` : ""};`;
});
...
var diff_exports = /* @__PURE__ */ require_chunk.__exportAll({ ddlDiff: () => ddlDiff });
const ddlDiff = async (ddl1, ddl2, schemasResolver, enumsResolver, sequencesResolver, policyResolver, roleResolver, privilegesResolver, tablesResolver, columnsResolver, viewsResolver, uniquesResolver, indexesResolver, checksResolver, pksResolver, fksResolver, mode) => {
	...
	const jsonRenamedCheckConstraints = checkRenames.map((it) => prepareStatement("rename_constraint", {
		schema: it.to.schema,
		table: it.to.table,
		from: it.from.name,
		to: it.to.name
	...
	const viewsAlters = alters.filter((it) => {
		...
		if (it.using && !it.using.to && it.using.from === require_grammar.defaults.accessMethod) delete it.using;
...
//#endregion
Object.defineProperty(exports, 'diff_exports', {
  enumerable: true,
  get: function () {
    return diff_exports;
  }
