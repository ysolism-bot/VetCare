...
__export(main_exports, {
  ...
  applyMigrations: () => import_database_dev2.applyMigrations,
  initializeTrackingTable: () => import_database_dev2.initializeTrackingTable,
  resetDatabase: () => import_database_dev2.resetDatabase
});
...
var import_database_dev = require("@netlify/database-dev");
var import_database_dev2 = require("@netlify/database-dev");
  ...
  async start() {
    ...
    if (this.#features.database) {
      try {
        ...
        const db = new import_database_dev.NetlifyDB({ directory: dbDirectory });
