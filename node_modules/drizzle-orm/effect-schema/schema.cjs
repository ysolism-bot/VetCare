...
function isOptional(schema) {
	...
	return effect.Schema.isSchema(schema) && schema.ast?.context?.isOptional === true;
}
function isStructField(schema) {
	if (effect.Schema.isSchema(schema)) return true;
...
function handleColumns(columns, refinements, conditions) {
	...
	for (const [key, selected] of Object.entries(columns)) {
		...
		const schema = column ? require_effect_schema_column.columnToSchema(column) : effect.Schema.Any;
		...
		if (column) {
			if (conditions.nullable(column)) columnSchemas[key] = effect.Schema.NullOr(columnSchemas[key]);
			if (conditions.optional(column)) columnSchemas[key] = effect.Schema.optional(effect.Schema.UndefinedOr(columnSchemas[key]));
		}
	}
	return effect.Schema.Struct(columnSchemas);
}
function handleEnum(enum_) {
	return effect.Schema.Literals(enum_.enumValues);
}
