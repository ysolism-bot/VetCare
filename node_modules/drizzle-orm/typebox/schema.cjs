...
function handleColumns(columns, refinements, conditions, factory) {
	...
	for (const [key, selected] of Object.entries(columns)) {
		...
		const schema = column ? require_typebox_column.columnToSchema(column, factory?.typeboxInstance ?? typebox.Type) : typebox.Type.Any();
		const refined = typeof refinement === "function" ? refinement(schema) : schema;
		if (conditions.never(column)) continue;
		else columnSchemas[key] = refined;
		if (column) {
			if (conditions.nullable(column)) columnSchemas[key] = typebox.Type.Union([columnSchemas[key], typebox.Type.Null()]);
			if (conditions.optional(column)) columnSchemas[key] = typebox.Type.Optional(columnSchemas[key]);
		}
	}
	return typebox.Type.Object(columnSchemas);
}
function handleEnum(enum_, factory) {
	return (factory?.typeboxInstance ?? typebox.Type).Enum(require_typebox_column.mapEnumValues(enum_.enumValues));
}
