...
var NodeCockroachPreparedQuery = class extends __cockroach_core_session_ts.CockroachPreparedQuery {
	...
	queryConfig;
	jitMapper;
	constructor(client, queryString, params, logger, fields, name, useJitMappers, customResultMapper) {
		super({
			sql: queryString,
			params
		});
	...
	async execute(placeholderValues = {}) {
		return __tracing_ts.tracer.startActiveSpan("drizzle.execute", async () => {
			...
			return __tracing_ts.tracer.startActiveSpan("drizzle.mapResponse", () => {
				if (customResultMapper) return customResultMapper(result.rows);
				return this.useJitMappers ? (this.jitMapper ??= (0, __utils_ts.makeJitQueryMapper)(fields, joinsNotNullableMap))(result.rows) : result.rows.map((row) => (0, __utils_ts.mapResultRow)(fields, row, joinsNotNullableMap));
			});
		});
	}
...
var NodeCockroachSession = class NodeCockroachSession extends __cockroach_core_session_ts.CockroachSession {
	...
	prepareQuery(query, fields, name, customResultMapper) {
		return new NodeCockroachPreparedQuery(this.client, query.sql, query.params, this.logger, fields, name, this.options.useJitMappers, customResultMapper);
	}
