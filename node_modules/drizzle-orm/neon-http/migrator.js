...
async function migrate(db, config) {
	...
	if (typeof config === "object" && config.init) {
		...
		if (!migration) return;
		await db.session.execute(sql`insert into ${sql.identifier(migrationsSchema)}.${sql.identifier(migrationsTable)} ("hash", "created_at", "name") values(${migration.hash}, ${migration.folderMillis}, ${migration.name})`);
		return;
	}
	...
	for await (const migration of migrationsToRun) {
		...
		rowsToInsert.push(sql`insert into ${sql.identifier(migrationsSchema)}.${sql.identifier(migrationsTable)} ("hash", "created_at", "name") values(${migration.hash}, ${migration.folderMillis}, ${migration.name})`);
	}
