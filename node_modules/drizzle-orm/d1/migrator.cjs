...
async function migrate(db, config) {
	...
	if (typeof config === "object" && config.init) {
		...
		if (!migration) return;
		await db.run(__sql_sql_ts.sql`INSERT INTO ${__sql_sql_ts.sql.identifier(migrationsTable)} ("hash", "created_at", "name", "applied_at") values(${migration.hash}, ${migration.folderMillis}, ${migration.name}, ${(/* @__PURE__ */ new Date()).toISOString()})`.inlineParams());
		return;
	}
	...
	for (const migration of migrationsToRun) {
		...
		statementToBatch.push(db.run(__sql_sql_ts.sql`INSERT INTO ${__sql_sql_ts.sql.identifier(migrationsTable)} ("hash", "created_at", "name", "applied_at") values(${migration.hash}, ${migration.folderMillis}, ${migration.name}, ${(/* @__PURE__ */ new Date()).toISOString()})`.inlineParams()));
	}
