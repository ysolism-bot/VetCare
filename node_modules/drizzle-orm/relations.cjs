...
function makeJitRqbMapperInner(selection, rowExpr, selectionVar, mapColumnValue, parseJson, parseJsonIfString, useJsonMappers, preFn, counter, accessByIdx) {
	...
	for (const [idx, { field, key, codec, isArray, selection: innerSelection, arrayDimensions }] of selection.entries()) {
		...
		} else if (require_entity.is(field, __table_ts.Table) || require_entity.is(field, require_sql_sql.View)) {} else {
...
function makeJitRqbMapper({ selection, isFirst, parseJson, parseJsonIfString, rootJsonMappers, arrayModeRoot }, mapColumnValue) {
	...
	return Object.assign(new require_utils.FnConstructor("rows", compiled).bind({
		selection,
		mapColumnValue
...
function createRelationsHelper(tables) {
	...
	const relationsTables = Object.entries(tables).reduce((acc, [tKey, value]) => {
		...
		const columns = Object.entries(value[__table_ts.TableColumns]).reduce((acc, [cKey, column]) => {
...
function relationsFilterToSQL(table, filter, tableRelations = {}, tablesRelations = {}, depth = 0) {
	...
	for (const [target, value] of entries) {
		...
		switch (target) {
			...
			case "OR":
				if (!value?.length) continue;
				parts.push(require_sql_expressions_conditions.or(...value.map((subFilter) => relationsFilterToSQL(table, subFilter, tableRelations, tablesRelations, depth))));
				continue;
			case "AND":
				if (!value?.length) continue;
				parts.push(require_sql_expressions_conditions.and(...value.map((subFilter) => relationsFilterToSQL(table, subFilter, tableRelations, tablesRelations, depth))));
				continue;
			case "NOT": {
				if (value === void 0) continue;
			...
			default: {
				...
				const filter = require_sql_expressions_conditions.and(relationFilter, typeof value === "boolean" ? void 0 : relationsFilterToSQL(targetTable, value, targetConfig.relations, tablesRelations, depth + 1));
				const subquery = throughTable ? require_sql_sql.sql`(select * from ${getTableAsAliasSQL(targetTable)} inner join ${getTableAsAliasSQL(throughTable)} on ${joinCondition}${require_sql_sql.sql` where ${filter}`.if(filter)} limit 1)` : require_sql_sql.sql`(select * from ${getTableAsAliasSQL(targetTable)}${require_sql_sql.sql` where ${filter}`.if(filter)} limit 1)`;
				if (filter) parts.push((value ? require_sql_expressions_conditions.exists : require_sql_expressions_conditions.notExists)(subquery));
			}
		}
	}
	return require_sql_expressions_conditions.and(...parts);
...
function relationExtrasToSQL(table, extras, codecs, inJson) {
	...
	for (const [key, field] of Object.entries(extras)) {
		...
		const column = codecs ? require_utils.getColumnFromDecoder(subq) : void 0;
		const query = column && (!inJson || !column.jsonSelectIdentifier) ? require_sql_sql.sql`${codecs.apply(column, inJson ? "castInJson" : "cast", require_sql_sql.sql`(${subq})`)} as ${require_sql_sql.sql.identifier(key)}` : require_sql_sql.sql`(${subq}) as ${require_sql_sql.sql.identifier(key)}`;
...
function relationToSQL(relation, sourceTable, targetTable, throughTable) {
	if (relation.through) {
		const outerColumnWhere = relation.sourceColumns.map((s, i) => {
			...
			return require_sql_expressions_conditions.eq(require_sql_sql.sql`${sourceTable}.${require_sql_sql.sql.identifier(s.name)}`, require_sql_sql.sql`${throughTable}.${require_sql_sql.sql.identifier(require_entity.is(t._.column, require_column.Column) ? t._.column.name : t._.key)}`);
		});
		const innerColumnWhere = relation.targetColumns.map((s, i) => {
			const t = relation.through.target[i];
			return require_sql_expressions_conditions.eq(require_sql_sql.sql`${throughTable}.${require_sql_sql.sql.identifier(require_entity.is(t._.column, require_column.Column) ? t._.column.name : t._.key)}`, require_sql_sql.sql`${targetTable}.${require_sql_sql.sql.identifier(s.name)}`);
		});
		return {
	...
	return { filter: require_sql_expressions_conditions.and(...relation.sourceColumns.map((s, i) => {
		...
		return require_sql_expressions_conditions.eq(require_sql_sql.sql`${sourceTable}.${require_sql_sql.sql.identifier(s.name)}`, require_sql_sql.sql`${targetTable}.${require_sql_sql.sql.identifier(t.name)}`);
...
exports.makeDefaultRqbMapper = makeDefaultRqbMapper;
exports.makeJitRqbMapper = makeJitRqbMapper;
exports.mapRelationalRow = mapRelationalRow;
exports.mapRelationalRowFromArrays = mapRelationalRowFromArrays;
