...
var CockroachSelectBuilder = class {
	static [entityKind] = "CockroachSelectBuilder";
	fields;
	session;
	dialect;
	withList = [];
	...
	*/
	from(source) {
		...
		else if (is(src, CockroachViewBase)) fields = src[ViewBaseConfig].selectedFields;
...
var CockroachSelectQueryBuilderBase = class extends TypedQueryBuilder {
	static [entityKind] = "CockroachSelectQueryBuilder";
	_;
	config;
	joinsNotNullableMap;
	tableName;
...
var CockroachSelectBase = class extends CockroachSelectQueryBuilderBase {
	static [entityKind] = "CockroachSelect";
	/** @internal */
	_prepare(name, generateName = false) {
		...
		return tracer.startActiveSpan("drizzle.prepareQuery", () => {
			...
			const preparedQuery = session.prepareQuery(query, fieldsList, name ?? (generateName ? preparedStatementName(query.sql, query.params) : name));
...
applyMixins(CockroachSelectBase, [QueryPromise]);
