...
var CockroachUpdateBuilder = class {
	static [__entity_ts.entityKind] = "CockroachUpdateBuilder";
	...
	set(values) {
		return new CockroachUpdateBase(this.table, (0, __utils_ts.mapUpdateSet)(this.table, values), this.session, this.dialect, this.withList);
	}
};
var CockroachUpdateBase = class extends __query_promise_ts.QueryPromise {
	static [__entity_ts.entityKind] = "CockroachUpdate";
	config;
	tableName;
	joinsNotNullableMap;
	...
	getTableLikeFields(table) {
		if ((0, __entity_ts.is)(table, __cockroach_core_table_ts.CockroachTable)) return table[__table_ts.Table.Symbol.Columns];
	...
	*/
		...
		if (!fields) {
			...
			if (this.config.from) {
				...
				for (const join of this.config.joins) {
					const tableName = (0, __utils_ts.getTableLikeName)(join.table);
					if (typeof tableName === "string" && !(0, __entity_ts.is)(join.table, __sql_sql_ts.SQL)) {
	...
	_prepare(name, generateName = false) {
		...
		const preparedQuery = this.session.prepareQuery(query, this.config.returning, name ?? (generateName ? (0, __query_name_generator_ts.preparedStatementName)(query.sql, query.params) : name));
...
//#endregion
exports.CockroachUpdateBase = CockroachUpdateBase;
exports.CockroachUpdateBuilder = CockroachUpdateBuilder;
