...
let __cockroach_core_view_base_ts = require("../view-base.cjs");

//#region src/cockroach-core/query-builders/select.ts
var CockroachSelectBuilder = class {
	static [__entity_ts.entityKind] = "CockroachSelectBuilder";
	fields;
	session;
	dialect;
	withList = [];
	...
	*/
	from(source) {
		...
		else if ((0, __entity_ts.is)(src, __cockroach_core_view_base_ts.CockroachViewBase)) fields = src[__view_common_ts.ViewBaseConfig].selectedFields;
		else if ((0, __entity_ts.is)(src, __sql_sql_ts.SQL)) fields = {};
		else fields = (0, __utils_ts.getTableColumns)(src);
		return new CockroachSelectBase({
			table: src,
			fields,
			isPartialSelect,
...
var CockroachSelectQueryBuilderBase = class extends __query_builders_query_builder_ts.TypedQueryBuilder {
	static [__entity_ts.entityKind] = "CockroachSelectQueryBuilder";
	_;
	config;
	joinsNotNullableMap;
	tableName;
	...
	*/
	...
	createSetOperator(type, isAll) {
		return (rightSelection) => {
			const rightSelect = typeof rightSelection === "function" ? rightSelection(getCockroachSetOperators()) : rightSelection;
	...
	*/
	...
	as(alias) {
		return new Proxy(new __subquery_ts.Subquery(this.getSQL(), this.config.fields, alias), new __selection_proxy_ts.SelectionProxyHandler({
			alias,
			sqlAliasedBehavior: "alias",
			sqlBehavior: "error"
		}));
...
var CockroachSelectBase = class extends CockroachSelectQueryBuilderBase {
	static [__entity_ts.entityKind] = "CockroachSelect";
	/** @internal */
	_prepare(name, generateName = false) {
		...
		return __tracing_ts.tracer.startActiveSpan("drizzle.prepareQuery", () => {
			const fieldsList = (0, __utils_ts.orderSelectedFields)(config.fields);
			const query = dialect.sqlToQuery(this.getSQL());
			const preparedQuery = session.prepareQuery(query, fieldsList, name ?? (generateName ? (0, __query_name_generator_ts.preparedStatementName)(query.sql, query.params) : name));
...
(0, __utils_ts.applyMixins)(CockroachSelectBase, [__query_promise_ts.QueryPromise]);
...
const getCockroachSetOperators = () => ({
	union,
	unionAll,
	intersect,
	intersectAll,
...
//#endregion
exports.CockroachSelectBase = CockroachSelectBase;
exports.CockroachSelectBuilder = CockroachSelectBuilder;
exports.CockroachSelectQueryBuilderBase = CockroachSelectQueryBuilderBase;
