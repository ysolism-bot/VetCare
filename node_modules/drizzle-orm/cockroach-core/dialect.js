...
import { upgradeIfNeeded } from "../up-migrations/cockroach.js";
...
var CockroachDialect = class {
	...
	async migrate(migrations, session, config) {
		...
		const dbMigrations = await session.all(sql`select id, hash, created_at, name from ${sql.identifier(migrationsSchema)}.${sql.identifier(migrationsTable)}`);
		if (typeof config === "object" && config.init) {
			...
			if (!migration) return;
			await session.execute(sql`insert into ${sql.identifier(migrationsSchema)}.${sql.identifier(migrationsTable)} ("hash", "created_at", "name") 
			...
			if (is(field, SQL.Aliased) && field.isSelectionField) {
				...
				if (isSingleTable) {
					const newSql = new SQL(query.queryChunks.map((c) => {
						if (is(c, CockroachColumn)) return sql.identifier(c.name);
						return c;
					}));
