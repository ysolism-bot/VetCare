...
let __up_migrations_cockroach_ts = require("../up-migrations/cockroach.cjs");
...
var CockroachDialect = class {
	...
	async migrate(migrations, session, config) {
		...
		const { newDb } = await (0, __up_migrations_cockroach_ts.upgradeIfNeeded)(migrationsSchema, migrationsTable, session, migrations);
		if (newDb) {
		...
		const dbMigrations = await session.all(__sql_sql_ts.sql`select id, hash, created_at, name from ${__sql_sql_ts.sql.identifier(migrationsSchema)}.${__sql_sql_ts.sql.identifier(migrationsTable)}`);
		if (typeof config === "object" && config.init) {
			...
			if (!migration) return;
			await session.execute(__sql_sql_ts.sql`insert into ${__sql_sql_ts.sql.identifier(migrationsSchema)}.${__sql_sql_ts.sql.identifier(migrationsTable)} ("hash", "created_at", "name") 
	...
	*/
			...
			if ((0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased) && field.isSelectionField) {
				...
				if (isSingleTable) {
					const newSql = new __sql_sql_ts.SQL(query.queryChunks.map((c) => {
						if ((0, __entity_ts.is)(c, __cockroach_core_columns_index_ts.CockroachColumn)) return __sql_sql_ts.sql.identifier(c.name);
						return c;
					}));
	...
	buildRelationalQueryWithoutPK({ fullSchema, schema, tableNamesMap, table, tableConfig, queryConfig: config, tableAlias, nestedQueryRelation, joinOn }) {
		...
		if (nestedQueryRelation) {
			let field = __sql_sql_ts.sql`json_build_array(${__sql_sql_ts.sql.join(selection.map(({ field, tsKey, isJson }) => isJson ? __sql_sql_ts.sql`${__sql_sql_ts.sql.identifier(`${tableAlias}_${tsKey}`)}.${__sql_sql_ts.sql.identifier("data")}` : (0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased) ? field.sql : field), __sql_sql_ts.sql`, `)})`;
