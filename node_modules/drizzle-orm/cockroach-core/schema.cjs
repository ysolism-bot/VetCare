...
var CockroachSchema = class {
	...
	constructor(schemaName, casing) {
		...
		this.table = Object.assign(this.table, { withRLS: ((name, columns, extraConfig) => {
			const table = require_cockroach_core_table.cockroachTableWithSchema(name, columns, extraConfig, this.schemaName, this.casing);
			table[require_cockroach_core_table.EnableRLS] = true;
			return table;
		}) });
	}
	table = ((name, columns, extraConfig) => {
		return require_cockroach_core_table.cockroachTableWithSchema(name, columns, extraConfig, this.schemaName, this.casing);
	});
	view = ((name, columns) => {
		return require_cockroach_core_view.cockroachViewWithSchema(name, columns, this.schemaName, this.casing);
	});
	materializedView = ((name, columns) => {
		return require_cockroach_core_view.cockroachMaterializedViewWithSchema(name, columns, this.schemaName, this.casing);
	});
	enum(enumName, input) {
