import { CockroachColumn, ExtraConfigColumn } from "./cockroach-core/columns/common.cjs";
import { MsSqlColumn } from "./mssql-core/columns/common.cjs";
import { MySqlColumn } from "./mysql-core/columns/common.cjs";
import { PgSequenceOptions } from "./pg-core/sequence.cjs";
import { ExtraConfigColumn as ExtraConfigColumn$1, PgColumn } from "./pg-core/columns/common.cjs";
import { SingleStoreColumn } from "./singlestore-core/columns/common.cjs";
import { Assume } from "./utils.cjs";
import { SQL } from "./sql/sql.cjs";
import { SQLiteColumn } from "./sqlite-core/columns/common.cjs";
...
type Dialect = 'pg' | 'mysql' | 'sqlite' | 'singlestore' | 'mssql' | 'common' | 'cockroach';
...
type BuildColumn<TTableName extends string, TBuilder extends ColumnBuilderBase, TDialect extends Dialect, TBuiltConfig extends ColumnBaseConfig<ColumnType> = MakeColumnConfig<TBuilder['_'], TTableName, TDialect>> = TDialect extends 'mysql' ? MySqlColumn<TBuiltConfig, {}> : TDialect extends 'mssql' ? MsSqlColumn<TBuiltConfig, {}> : TDialect extends 'sqlite' ? SQLiteColumn<TBuiltConfig, {}> : TDialect extends 'singlestore' ? SingleStoreColumn<TBuiltConfig, {}> : TDialect extends 'cockroach' ? CockroachColumn<TBuiltConfig, {}> : TDialect extends 'common' ? Column<TBuiltConfig, {}> : never;
type BuildIndexColumn<TDialect extends Dialect> = TDialect extends 'pg' ? ExtraConfigColumn$1 : TDialect extends 'cockroach' ? ExtraConfigColumn : never;
...
type ChangeColumnTableName<TColumn extends Column, TAlias extends string, TDialect extends Dialect> = TDialect extends 'pg' ? PgColumn<TColumn['_']['dataType'], Omit<TColumn['_'], 'tableName'> & {
  tableName: TAlias;
  insertType: unknown;
}> : TDialect extends 'mysql' ? MySqlColumn<MakeColumnConfig<TColumn['_'], TAlias>> : TDialect extends 'singlestore' ? SingleStoreColumn<MakeColumnConfig<TColumn['_'], TAlias>> : TDialect extends 'sqlite' ? SQLiteColumn<MakeColumnConfig<TColumn['_'], TAlias>> : TDialect extends 'mssql' ? MsSqlColumn<MakeColumnConfig<TColumn['_'], TAlias>> : TDialect extends 'cockroach' ? CockroachColumn<MakeColumnConfig<TColumn['_'], TAlias>> : never;
//#endregion
...
//# sourceMappingURL=column-builder.d.cts.map