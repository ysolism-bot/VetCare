...
import { OriginalName, Table } from "../../table.js";
...
`;
var UpstashCache = class UpstashCache extends Cache {
	...
	*/
	...
	async onMutate(params) {
		...
		const compositeTableSets = (Array.isArray(params.tables) ? params.tables : params.tables ? [params.tables] : []).map((table) => is(table, Table) ? table[OriginalName] : table).map((table) => this.addTablePrefix(table));
...
function upstashCache({ url, token, config, global = false }) {
	return new UpstashCache(new Redis({
		url,
		token
	}), config, global);
}
