...
const upgradeFunctions = { 0: (migrationsSchema, migrationsTable, session, localMigrations) => {
	return effect.Effect.gen(function* () {
		...
		yield* session.transaction((tx) => effect.Effect.gen(function* () {
			yield* tx.execute(__sql_sql_ts.sql`ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS ${__sql_sql_ts.sql.identifier("name")} text`);
			yield* tx.execute(__sql_sql_ts.sql`ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS ${__sql_sql_ts.sql.identifier("applied_at")} timestamp with time zone DEFAULT now()`);
			for (const backfillEntry of toApply) yield* tx.execute(__sql_sql_ts.sql`UPDATE ${table} SET ${__sql_sql_ts.sql.identifier("name")} = ${backfillEntry.name}, ${__sql_sql_ts.sql.identifier("applied_at")} = NULL WHERE ${__sql_sql_ts.sql.identifier("id")} = ${backfillEntry.id}`);
		}));
	});
} };
/**
...
const upgradeIfNeeded = effect.Effect.fn("upgradeIfNeeded")(function* (migrationsSchema, migrationsTable, session, localMigrations) {
	...
	const rows = yield* session.objects(__sql_sql_ts.sql`SELECT
			n.nspname AS "schema",
...
//# sourceMappingURL=effect-pg.cjs.map