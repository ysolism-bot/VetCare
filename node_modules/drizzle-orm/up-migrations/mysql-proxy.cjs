...
const upgradeFunctions = { 0: async (migrationsTable, db, callback, localMigrations) => {
	...
	const dbRows = (await db.session.arrays(__sql_sql_ts.sql`SELECT id, hash, created_at FROM ${table} ORDER BY id ASC`)).map((it) => ({
		id: it[0],
		hash: it[1],
		created_at: it[2]
	}));
...
async function upgradeIfNeeded(migrationsTable, db, callback, localMigrations) {
	if ((await db.session.arrays(__sql_sql_ts.sql`SELECT 1 FROM information_schema.tables 
			WHERE table_name = ${migrationsTable}
			AND table_schema = DATABASE()`)).length === 0) return { newDb: true };
	const rows = await db.session.arrays(__sql_sql_ts.sql`SELECT column_name as \`column_name\`
