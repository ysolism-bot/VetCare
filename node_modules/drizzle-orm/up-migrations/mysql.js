...
const upgradeFunctions = { 0: async (migrationsTable, session, localMigrations) => {
	...
	const dbRows = await session.objects(sql`SELECT id, hash, created_at FROM ${table} ORDER BY id ASC`);
...
async function upgradeIfNeeded(migrationsTable, session, localMigrations) {
	if ((await session.objects(sql`SELECT 1 FROM information_schema.tables 
			WHERE table_name = ${migrationsTable}
			AND table_schema = DATABASE()`)).length === 0) return { newDb: true };
	const rows = await session.objects(sql`SELECT column_name as \`column_name\`
