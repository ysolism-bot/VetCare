...
let __pg_core_query_builders_select_ts = require("../query-builders/select.cjs");
	...
	*/
	with(...queries) {
		...
		function select(fields) {
			return new __pg_core_query_builders_select_ts.PgSelectBuilder({
				fields: fields ?? void 0,
				session: self.session,
				dialect: self.dialect,
				withList: queries
			}, __pg_core_effect_select_ts.PgEffectSelectBase);
		}
