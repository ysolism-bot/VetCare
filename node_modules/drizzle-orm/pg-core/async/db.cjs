...
var PgAsyncDatabase = class {
	...
	*/
	$with = (alias, selection) => {
		const as = (qb) => {
			if (typeof qb === "function") qb = qb(new __pg_core_query_builders_index_ts.QueryBuilder(this.dialect));
	...
	*/
	with(...queries) {
		...
		function select(fields) {
			return new __pg_core_query_builders_index_ts.PgSelectBuilder({
				fields: fields ?? void 0,
				session: self.session,
				dialect: self.dialect,
				withList: queries,
				tagged: self.tagged
			}, require_pg_core_async_select.PgAsyncSelectBase);
		}
