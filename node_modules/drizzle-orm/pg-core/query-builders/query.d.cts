...
import { Query, SQL, SQLWrapper, SqlCommenterInput } from "../../sql/sql.cjs";
...
interface PgRelationalQueryConstructor {
  new (schema: TablesRelationalConfig, table: PgTable, tableConfig: TableRelationalConfig, dialect: PgDialect, session: PgSession, config: DBQueryConfigWithComment<'many' | 'one'> | true, mode: 'many' | 'first', parseJson: boolean): AnyPgRelationalQuery;
}
...
declare class RelationalQueryBuilder<TSchema extends TablesRelationalConfig, TFields extends TableRelationalConfig, TBuilderHKT extends PgRelationalQueryHKTBase = PgRelationalQueryHKT> {
  ...
  findFirst<TConfig extends DBQueryConfigWithComment<'one', TSchema, TFields>>(config?: KnownKeysOnly<TConfig, DBQueryConfigWithComment<'one', TSchema, TFields>> & {
    comment?: SqlCommenterInput;
