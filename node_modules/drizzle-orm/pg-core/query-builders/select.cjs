...
var PgSelectBuilder = class {
	static [__entity_ts.entityKind] = "PgSelectBuilder";
	fields;
	session;
	dialect;
	withList = [];
...
var PgSelectBase = class extends __query_builders_query_builder_ts.TypedQueryBuilder {
	...
	constructor(config) {
		...
		this.tableName = (0, __utils_ts.getTableLikeName)(config.table);
		this.joinsNotNullableMap = typeof this.tableName === "string" ? { [this.tableName]: true } : {};
		for (const item of require_pg_core_utils.extractUsedTable(config.table)) this.usedTables.add(item);
		this.config.withList?.forEach((it) => {
			const extracted = require_pg_core_utils.extractUsedTable(it);
	...
	*/
	...
	as(alias) {
		...
		return new Proxy(new __subquery_ts.Subquery(this.withoutSelectionCastCodecs().getSQL(), this.config.fields, alias, false, [...new Set(usedTables)]), new __selection_proxy_ts.SelectionProxyHandler({
			alias,
			sqlAliasedBehavior: "alias",
			sqlBehavior: "error"
		}));
