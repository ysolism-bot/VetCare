...
declare namespace sql {
   ...
   */
  function comment(input: CommentInput): SQL | undefined;
}
declare function sqlCommenter(input: CommentInput): string;
declare namespace sqlCommenter {
  function merge(input1: CommentInput | undefined, input2: CommentInput | undefined): string;
  function encodeInput(input: CommentInput): string;
  function sanitizeObjectElement(key: string): string;
  function sanitizeStringInput(input: string): string;
}
type CommentInput = string | SqlCommenterInput;
type SqlCommenterInput = Record<string, string | number | bigint | boolean | null | undefined>;
declare namespace SQL {
...
//#endregion
export { BuildQueryConfig, Chunk, ColumnsSelection, CommentInput, DriverValueDecoder, DriverValueDecoderFn, DriverValueEncoder, DriverValueEncoderFn, DriverValueMapper, FakePrimitiveParam, GetDecoderResult, InferSelectViewModel, Name, Param, Placeholder, Query, SQL, SQLChunk, SQLEntity, SQLGenerator, SQLWrapper, SqlCommenterInput, StringChunk, View, fillPlaceholders, getViewName, isDriverValueEncoder, isNoop, isSQLWrapper, isView, name, noopDecoder, noopEncoder, noopMapper, param, placeholder, sql, sqlCommenter };
