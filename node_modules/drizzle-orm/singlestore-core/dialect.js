	...
	*/
			...
			if (is(field, SQL.Aliased) && field.isSelectionField) {
				...
				if (isSingleTable) {
					const newSql = new SQL(query.queryChunks.map((c) => {
						if (is(c, SingleStoreColumn)) return sql.identifier(c.name);
						return c;
					}));
	...
	_buildRelationalQuery({ fullSchema, schema, tableNamesMap, table, tableConfig, queryConfig: config, tableAlias, nestedQueryRelation, joinOn }) {
		...
		if (nestedQueryRelation) {
			let field = sql`JSON_TO_ARRAY(${sql.join(selection.map(({ field, tsKey, isJson }) => isJson ? sql`${sql.identifier(`${tableAlias}_${tsKey}`)}.${sql.identifier("data")}` : is(field, SQL.Aliased) ? field.sql : field), sql`, `)})`;
	...
	buildRqbColumn(table, column, key) {
		if (is(column, Column)) {
			...
			switch (column.columnType) {
				...
				case "SingleStoreEnumColumn": return sql`cast(${name} as char) as ${sql.identifier(key)}`;
				case "SingleStoreVector":
				case "SingleStoreBigIntVector": return sql`hex(${name} :> blob) as ${sql.identifier(key)}`;
				case "SingleStoreCustomColumn": return sql`${column.jsonSelectIdentifier(name, sql)} as ${sql.identifier(key)}`;
	...
	buildRelationalQuery({ schema, table, tableConfig, queryConfig: config, relationWhere, mode, errorPath, depth, isNestedMany, throughJoin }) {
		...
		const joins = params ? (() => {
			...
			return sql.join(withEntries.map(([k, join]) => {
				...
				return sql` left join lateral(select ${sql`${isSingle ? sql`json_build_object(${jsonColumns})` : sql`json_agg(json_build_object(${jsonColumns})${innerQuery.order ? sql` ORDER BY ${sql.identifier(`$drizzle_order_row_number`)}` : void 0})`} as ${sql.identifier("r")}`} from (${innerQuery.sql}) as ${sql.identifier("t")}) as ${sql.identifier(k)} on true`;
			}));
		})() : void 0;
		...
		return {
			sql: sql`select ${sql.join(selectionArr, sql`, `)} from ${getTableAsAliasSQL(table)}${throughJoin}${sql`${joins}`.if(joins)}${sql` where ${where}`.if(where)}${sql` order by ${order}`.if(order)}${sql` limit ${limit}`.if(limit !== void 0)}${sql` offset ${offset}`.if(offset !== void 0)}`,
			selection,
			order
		};
	}
