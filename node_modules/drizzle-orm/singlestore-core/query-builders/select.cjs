...
//#region src/singlestore-core/query-builders/select.ts
var SingleStoreSelectBuilder = class {
	static [__entity_ts.entityKind] = "SingleStoreSelectBuilder";
	fields;
	session;
	dialect;
	withList = [];
...
var SingleStoreSelectQueryBuilderBase = class extends __query_builders_query_builder_ts.TypedQueryBuilder {
	static [__entity_ts.entityKind] = "SingleStoreSelectQueryBuilder";
	_;
	config;
	joinsNotNullableMap;
	tableName;
	...
	constructor({ table, fields, isPartialSelect, session, dialect, withList, distinct }) {
		...
		for (const item of require_singlestore_core_utils.extractUsedTable(table)) this.usedTables.add(item);
	}
	/** @internal */
	getUsedTables() {
		...
		return (table, on) => {
			...
			if (!this.isPartialSelect) {
				...
				if (typeof tableName === "string" && !(0, __entity_ts.is)(table, __sql_sql_ts.SQL)) {
					const selection = (0, __entity_ts.is)(table, __subquery_ts.Subquery) ? table._.selectedFields : table[__table_ts.Table.Symbol.Columns];
	...
	*/
	...
	as(alias) {
		const usedTables = [];
		usedTables.push(...require_singlestore_core_utils.extractUsedTable(this.config.table));
		if (this.config.joins) for (const it of this.config.joins) usedTables.push(...require_singlestore_core_utils.extractUsedTable(it.table));
...
var SingleStoreSelectBase = class extends SingleStoreSelectQueryBuilderBase {
	static [__entity_ts.entityKind] = "SingleStoreSelect";
	prepare() {
...
(0, __utils_ts.applyMixins)(SingleStoreSelectBase, [__query_promise_ts.QueryPromise]);
...
//#endregion
exports.SingleStoreSelectBase = SingleStoreSelectBase;
exports.SingleStoreSelectBuilder = SingleStoreSelectBuilder;
exports.SingleStoreSelectQueryBuilderBase = SingleStoreSelectQueryBuilderBase;
