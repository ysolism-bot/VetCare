...
import { BinaryOperator, and, arrayContained, arrayContains, arrayOverlaps, between, exists, ilike, inArray, isNotNull, isNull, like, not, notBetween, notExists, notIlike, notInArray, notLike, or } from "./sql/expressions/conditions.cjs";
import { asc, desc } from "./sql/expressions/select.cjs";
import { Assume, DrizzleTypeError, Equal, Simplify, ValueOrArray } from "./utils.cjs";
import { CodecsCollection, NormalizeArrayCodec, NormalizeCodec } from "./codecs.cjs";
import { CommentInput, Placeholder, SQL, SQLWrapper, View, sql } from "./sql/sql.cjs";
...
type DBQueryConfigWithComment<TRelationType extends 'one' | 'many' = 'one' | 'many', TSchema extends TablesRelationalConfig = TablesRelationalConfig, TTableConfig extends TableRelationalConfig = TableRelationalConfig> = DBQueryConfig<TRelationType, TSchema, TTableConfig> & {
  /**
...
interface BuildRelationalQueryResult {
  selection: {
    key: string;
    field: Column<any> | Table | View | SQL | SQL.Aliased | SQLWrapper | AggregatedField; /** For array type relations */
    isArray?: boolean;
    selection?: BuildRelationalQueryResult['selection'];
    isOptional?: boolean;
    codec?: NormalizeCodec | NormalizeArrayCodec; /** For array type columns */
    arrayDimensions?: number;
  }[];
  sql: SQL;
}
declare function mapRelationalRow(rows: Record<string, unknown> | Record<string, unknown>[], isOne: boolean, buildQueryResultSelection: BuildRelationalQueryResult['selection'], mapColumnValue?: (value: unknown) => unknown, /** Needed for SQLite as it returns JSON values as strings */

parseJson?: boolean, /** Needed for SingleStore as it returns JSON arrays as strings */

parseJsonIfString?: boolean, /** Root level data of query is usually not nested in JSON */

useJsonMappers?: boolean): Record<string, unknown> | Record<string, unknown>[];
declare function mapRelationalRowFromArrays(rows: unknown[][] | unknown[], isOne: boolean, buildQueryResultSelection: BuildRelationalQueryResult['selection'], mapColumnValue?: (value: unknown) => unknown, /** Needed for SQLite as it returns JSON values as strings */

parseJson?: boolean, /** Needed for SingleStore as it returns JSON arrays as strings */

parseJsonIfString?: boolean): Record<string, unknown> | Record<string, unknown>[];
interface RelationalRowsMapper<T = any> {
  (rows: unknown[][] | Record<string, unknown>[]): T;
}
type RelationalRowsMapperGenerator<T = any> = (config: RelationalQueryMapperConfig, mapColumnValue?: (value: unknown) => unknown) => RelationalRowsMapper<T>;
declare function makeDefaultRqbMapper<T = any>({
  selection,
  isFirst,
  parseJson,
  parseJsonIfString,
  rootJsonMappers,
  arrayModeRoot
}: RelationalQueryMapperConfig, mapColumnValue?: (value: unknown) => unknown): RelationalRowsMapper<T>;
interface RelationalQueryMapperConfig {
  selection: BuildRelationalQueryResult['selection'];
...
declare function makeJitRqbMapper<T = unknown>({
  selection,
  isFirst,
  parseJson,
  parseJsonIfString,
...
declare class RelationsBuilderColumn<TTableName extends string = string> implements RelationsBuilderColumnBase<TTableName> {
  ...
  constructor(column: FieldValue, tableName: TTableName, key: string);
...
declare class RelationsBuilderJunctionColumn<TTableName extends string = string> implements RelationsBuilderColumnBase<TTableName> {
  ...
  constructor(column: FieldValue, tableName: TTableName, key: string, through: RelationsBuilderColumnBase);
}
...
type RelationsBuilderColumns<TTable extends SchemaEntry, TTableName extends string> = { [TColumnName in keyof GetTableViewColumns<TTable>]: RelationsBuilderColumn<TTableName> };
...
type ExtractTablesFromSchema<TSchema extends Record<string, unknown>> = { [K in keyof TSchema as TSchema[K] extends SchemaEntry ? K extends string ? K : never : never]: Assume<TSchema[K], SchemaEntry> };
...
declare function relationsFilterToSQL(table: SchemaEntry, filter: AnyRelationsFilter | AnyTableFilter, tableRelations: RelationsRecord, tablesRelations: TablesRelationalConfig, depth?: number): SQL | undefined;
declare function relationsOrderToSQL(table: SchemaEntry, orders: OrderBy): SQL | undefined;
declare function relationExtrasToSQL(table: SchemaEntry, extras: Extras, codecs?: CodecsCollection, inJson?: boolean): {
...
declare function relationToSQL(relation: Relation, sourceTable: SchemaEntry, targetTable: SchemaEntry, throughTable?: SchemaEntry): BuiltRelationFilters;
declare function getTableAsAliasSQL(table: SchemaEntry): SQL<unknown>;
//#endregion
export { AggregatedField, AnyDBQueryConfig, AnyMany, AnyManyConfig, AnyOne, AnyOneConfig, AnyRelation, AnyRelations, AnyRelationsBuilderConfig, AnyRelationsFilter, AnyTableFilter, BuildQueryResult, BuildRelationResult, BuildRelationalQueryResult, BuiltRelationFilters, ColumnWithTSName, Count, DBQueryConfig, DBQueryConfigColumns, DBQueryConfigExtras, DBQueryConfigOrderBy, DBQueryConfigOrderByCallback, DBQueryConfigOrderByObject, DBQueryConfigWith, DBQueryConfigWithComment, EmptyRelations, ExtractObjectValues, ExtractTablesFromSchema, ExtractTablesWithRelations, ExtractTablesWithRelationsParts, Extras, FieldSelection, FieldValue, FilteredSchemaEntry, FindTargetTableInRelationalConfig, GetTableViewColumns, GetTableViewFieldSelection, IncludeEveryTable, InferRelationalQueryTableResult, Many, ManyConfig, ManyFn, NonUndefinedKeysOnly, One, OneConfig, OneFn, Operators, OrderBy, OrderByOperators, Relation, RelationFieldsFilterInternals, RelationResultKind, RelationalQueryMapperConfig, RelationalRowsMapper, RelationalRowsMapperGenerator, RelationsBuilder, RelationsBuilderColumn, RelationsBuilderColumnBase, RelationsBuilderColumnConfig, RelationsBuilderColumns, RelationsBuilderConfig, RelationsBuilderConfigValue, RelationsBuilderJunctionColumn, RelationsBuilderTable, RelationsBuilderTables, RelationsFieldFilter, RelationsFilter, RelationsFilterColumns, RelationsFilterCommons, RelationsFilterRelations, RelationsHelperStatic, RelationsOrder, RelationsRecord, ReturnTypeOrValue, SQLOperator, Schema, SchemaEntry, TableFilter, TableFilterColumns, TableFilterCommons, TableRelationalConfig, TablesRelationalConfig, TruthyKeysOnly, WithContainer, buildRelations, buildRelationsParts, createRelationsHelper, defineRelations, defineRelationsPart, extractTablesFromSchema, getOrderByOperators, getTableAsAliasSQL, makeDefaultRqbMapper, makeJitRqbMapper, mapRelationalRow, mapRelationalRowFromArrays, operators, orderByOperators, processRelations, relationExtrasToSQL, relationToSQL, relationsFilterToSQL, relationsOrderToSQL };
