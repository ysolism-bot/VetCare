...
let __mysql_core_table_ts = require("../table.cjs");
...
var MySqlSelectBuilder = class {
	static [__entity_ts.entityKind] = "MySqlSelectBuilder";
	fields;
	session;
	dialect;
	withList = [];
	...
	from(source, onIndex) {
		...
		else if ((0, __entity_ts.is)(source, require_mysql_core_view_base.MySqlViewBase)) fields = source[__view_common_ts.ViewBaseConfig].selectedFields;
		else if ((0, __entity_ts.is)(source, __sql_sql_ts.SQL)) fields = {};
		else fields = (0, __utils_ts.getTableColumns)(source);
		let useIndex = [];
		let forceIndex = [];
		let ignoreIndex = [];
		if ((0, __entity_ts.is)(source, __mysql_core_table_ts.MySqlTable) && onIndex && typeof onIndex !== "string") {
			if (onIndex.useIndex) useIndex = require_mysql_core_utils.convertIndexToString(require_mysql_core_utils.toArray(onIndex.useIndex));
			if (onIndex.forceIndex) forceIndex = require_mysql_core_utils.convertIndexToString(require_mysql_core_utils.toArray(onIndex.forceIndex));
			if (onIndex.ignoreIndex) ignoreIndex = require_mysql_core_utils.convertIndexToString(require_mysql_core_utils.toArray(onIndex.ignoreIndex));
		}
...
var MySqlSelectQueryBuilderBase = class extends __query_builders_query_builder_ts.TypedQueryBuilder {
	static [__entity_ts.entityKind] = "MySqlSelectQueryBuilder";
	_;
	config;
	joinsNotNullableMap;
	tableName;
	...
	constructor({ table, fields, isPartialSelect, session, dialect, withList, distinct, useIndex, forceIndex, ignoreIndex }) {
		...
		for (const item of require_mysql_core_utils.extractUsedTable(table)) this.usedTables.add(item);
	}
	/** @internal */
	getUsedTables() {
	...
	createJoin(joinType, lateral) {
		return (table, a, b) => {
			...
			let forceIndex = [];
			let ignoreIndex = [];
			if ((0, __entity_ts.is)(table, __mysql_core_table_ts.MySqlTable) && onIndex && typeof onIndex !== "string") {
				if (onIndex.useIndex) useIndex = require_mysql_core_utils.convertIndexToString(require_mysql_core_utils.toArray(onIndex.useIndex));
				if (onIndex.forceIndex) forceIndex = require_mysql_core_utils.convertIndexToString(require_mysql_core_utils.toArray(onIndex.forceIndex));
				if (onIndex.ignoreIndex) ignoreIndex = require_mysql_core_utils.convertIndexToString(require_mysql_core_utils.toArray(onIndex.ignoreIndex));
			}
	...
	*/
	...
	as(alias) {
		const usedTables = [];
		usedTables.push(...require_mysql_core_utils.extractUsedTable(this.config.table));
		if (this.config.joins) for (const it of this.config.joins) usedTables.push(...require_mysql_core_utils.extractUsedTable(it.table));
...
var MySqlSelectBase = class extends MySqlSelectQueryBuilderBase {
	static [__entity_ts.entityKind] = "MySqlSelect";
	prepare() {
...
(0, __utils_ts.applyMixins)(MySqlSelectBase, [__query_promise_ts.QueryPromise]);
...
//#endregion
exports.MySqlSelectBase = MySqlSelectBase;
exports.MySqlSelectBuilder = MySqlSelectBuilder;
exports.MySqlSelectQueryBuilderBase = MySqlSelectQueryBuilderBase;
