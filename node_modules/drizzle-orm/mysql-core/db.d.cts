import { MySqlDeleteBase } from "./query-builders/delete.cjs";
import { MySqlTable } from "./table.cjs";
import { MySqlViewBase } from "./view-base.cjs";
import { MySqlSelectBuilder } from "./query-builders/select.cjs";
import { MySqlUpdateBuilder } from "./query-builders/update.cjs";
import { MySqlInsertBuilder } from "./query-builders/insert.cjs";
...
import { MySqlQueryResultHKT, MySqlQueryResultKind, MySqlSession, MySqlTransaction, MySqlTransactionConfig } from "./session.cjs";
...
declare class MySqlDatabase<TQueryResult extends MySqlQueryResultHKT, TRelations extends AnyRelations = EmptyRelations> {
  ...
  session: MySqlSession<any, any>, relations: TRelations);
  /**
   ...
   */
  $with: WithBuilder;
  $count(source: MySqlTable | MySqlViewBase | SQL | SQLWrapper, filters?: SQL<unknown>): MySqlCountBuilder;
  $cache: {
  ...
  with(...queries: WithSubquery[]): {
    select: {
      ...
      <TSelection extends SelectedFields>(fields: TSelection): MySqlSelectBuilder<TSelection>;
    };
    selectDistinct: {
      (): MySqlSelectBuilder<undefined>;
      <TSelection extends SelectedFields>(fields: TSelection): MySqlSelectBuilder<TSelection>;
  ...
  selectDistinct<TSelection extends SelectedFields>(fields: TSelection): MySqlSelectBuilder<TSelection>;
  /**
  ...
  transaction<T>(transaction: (tx: MySqlTransaction<TQueryResult, TRelations>, config?: MySqlTransactionConfig) => Promise<T>, config?: MySqlTransactionConfig): Promise<T>;
}
type MySQLWithReplicas<Q> = Q & {
  $primary: Q;
  $replicas: Q[];
};
declare const withReplicas: <HKT extends MySqlQueryResultHKT, TRelations extends AnyRelations, Q extends MySqlDatabase<HKT, TRelations>>(primary: Q, replicas: [Q, ...Q[]], getReplica?: (replicas: Q[]) => Q) => MySQLWithReplicas<Q>;
//#endregion
