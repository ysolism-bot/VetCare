...
var LibSQLSession = class LibSQLSession extends __sqlite_core_session_ts.SQLiteSession {
	static [__entity_ts.entityKind] = "LibSQLSession";
	logger;
	cache;
...
var LibSQLTransaction = class LibSQLTransaction extends __sqlite_core_index_ts.SQLiteTransaction {
	static [__entity_ts.entityKind] = "LibSQLTransaction";
...
var LibSQLPreparedQuery = class extends __sqlite_core_session_ts.SQLitePreparedQuery {
	...
	mapAllResult(rows, isFromBatch) {
		...
		if (this.isRqbV2Query) return this.useJitMappers ? (this.jitMapper = this.jitMapper ?? (0, __relations_ts.makeJitRqbMapper)(this.rqbConfig, normalizeFieldValue))(rows) : this.customResultMapper(rows, normalizeFieldValue);
		if (this.customResultMapper) return this.customResultMapper(rows, normalizeFieldValue);
		return this.useJitMappers ? (this.jitMapper = this.jitMapper ?? (0, __utils_ts.makeJitQueryMapper)(this.fields, this.joinsNotNullableMap))(rows.map((row) => Array.prototype.slice.call(row).map((v) => normalizeFieldValue(v)))) : rows.map((row) => (0, __utils_ts.mapResultRow)(this.fields, row, this.joinsNotNullableMap));
	}
	...
	mapGetResult(rows, isFromBatch) {
		...
		if (!row) return;
		if (this.isRqbV2Query) return this.useJitMappers ? (this.jitMapper = this.jitMapper ?? (0, __relations_ts.makeJitRqbMapper)(this.rqbConfig, normalizeFieldValue))([row]) : this.customResultMapper([row], normalizeFieldValue);
		if (this.customResultMapper) return this.customResultMapper(rows, normalizeFieldValue);
...
//#endregion
exports.LibSQLPreparedQuery = LibSQLPreparedQuery;
