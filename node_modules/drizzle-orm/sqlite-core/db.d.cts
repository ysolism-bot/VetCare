...
import { SQLiteCountBuilderKind } from "./query-builders/count.cjs";
...
import { DBResult, Result, SQLiteSession, SQLiteTransaction, SQLiteTransactionConfig } from "./session.cjs";
...
declare class BaseSQLiteDatabase<TResultKind extends 'sync' | 'async', TRunResult, TFullSchema extends Record<string, unknown> = Record<string, never>, TRelations extends AnyRelations = EmptyRelations, TSchema extends V1.TablesRelationalConfig = V1.ExtractTablesWithRelations<TFullSchema>> {
   ...
   */
  $with: WithBuilder;
  $count(source: SQLiteTable | SQLiteViewBase | SQL | SQLWrapper, filters?: SQL<unknown>): SQLiteCountBuilderKind<TResultKind>;
  /**
  ...
  transaction<T>(transaction: (tx: SQLiteTransaction<TResultKind, TRunResult, TFullSchema, TRelations, TSchema>) => TResultKind extends 'sync' ? T extends Promise<any> ? DrizzleTypeError<"Sync drivers can't use async functions in transactions!"> : T : Result<TResultKind, T>, config?: SQLiteTransactionConfig): Result<TResultKind, T>;
}
