...
import { and, eq, isSQLWrapper as isSQLWrapper$1 } from "../sql/index.js";
	...
	*/
	...
	_buildRelationalQuery({ fullSchema, schema, tableNamesMap, table, tableConfig, queryConfig: config, tableAlias, nestedQueryRelation, joinOn }) {
		...
		if (nestedQueryRelation) {
			...
			if (is(nestedQueryRelation, V1.Many)) field = sql`coalesce(json_group_array(${field}), json_array())`;
	...
	buildRqbColumn(table, column, key) {
		if (is(column, Column)) {
			...
			switch (column.columnType) {
				case "SQLiteBigInt":
				case "SQLiteBlobJson":
				case "SQLiteBlobBuffer": return sql`hex(${name}) as ${sql.identifier(key)}`;
				case "SQLiteNumeric":
				case "SQLiteNumericNumber":
				case "SQLiteNumericBigInt": return sql`cast(${name} as text) as ${sql.identifier(key)}`;
				case "SQLiteCustomColumn": return sql`${column.jsonSelectIdentifier(name, sql)} as ${sql.identifier(key)}`;
...
var SQLiteAsyncDialect = class extends SQLiteDialect {
	...
	async migrate(migrations, db, config) {
		...
		const dbMigrations = await db.session.all(sql`SELECT id, hash, created_at, name FROM ${sql.identifier(migrationsTable)};`);
		if (typeof config === "object" && config.init) {
			if (dbMigrations.length) return { exitCode: "databaseMigrations" };
			if (migrations.length > 1) return { exitCode: "localMigrations" };
			const [migration] = migrations;
...
//#endregion
export { SQLiteAsyncDialect, SQLiteDialect, SQLiteSyncDialect };
