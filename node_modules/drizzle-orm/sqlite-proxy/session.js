...
var RemotePreparedQuery = class extends SQLitePreparedQuery {
	...
	mapGetResult(rows, isFromBatch) {
		...
		if (!row) return;
		if (this.isRqbV2Query) return this.useJitMappers ? (this.jitMapper = this.jitMapper ?? makeJitRqbMapper(this.rqbConfig))([JSON.parse(row)]) : this.customResultMapper([JSON.parse(row)]);
		if (this.customResultMapper) return this.customResultMapper([rows]);
