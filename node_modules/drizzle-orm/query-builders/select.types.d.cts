...
type AddAliasToSelection<TSelection extends ColumnsSelection, TAlias extends string, TDialect extends Dialect> = Simplify<IsAny<TSelection> extends true ? any : { [Key in keyof TSelection]: TSelection[Key] extends Column ? ChangeColumnTableName<TSelection[Key], TAlias, TDialect> : TSelection[Key] extends Table ? AddAliasToSelection<TSelection[Key]['_']['columns'], TAlias, TDialect> : TSelection[Key] extends SQL | SQL.Aliased ? TSelection[Key] : TSelection[Key] extends Subquery ? FromSingleKeyObject<TSelection[Key]['_']['selectedFields'], TSelection[Key]['_']['selectedFields'] extends {
  [key: string]: infer TValue;
} ? SQL.Aliased<SelectResultField<TValue>> : never, 'You can only select one column in the subquery'> : TSelection[Key] extends ColumnsSelection ? MapColumnsToTableAlias<TSelection[Key], TAlias, TDialect> : never }>;
